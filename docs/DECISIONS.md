...
| D20 | 2026-09-19 | Benchmark controls are the products' own compaction mechanisms, ported verbatim from open source (Codex, Gemini CLI, Grok Build, OpenCode, LangChain, Hermes, goose, Aider, AI SDK prune) plus Anthropic's compaction API; closed/opaque ones are listed without numbers | Edward: "the control should be the same vanilla compact mechanism used by each big name — that's the whole reason to be framework agnostic". A hand-written summarization prompt proved nothing about real products. | Generic truncation + generic summary only (v1) |
| D21 | 2026-09-19 | `votes` option (default 1): ask every question N times in parallel and average | Jev's P(keep) moves up to 0.16 between identical requests; two units sat at the 0.7 threshold and the compaction ratio ranged 53–76% across 12 runs. `votes: 3` halved the spread at negligible cost and no latency. Default stays 1 until real-transcript data justifies the 3× tokens. | Hysteresis; `jev-preview`; raising `dropThreshold` |
| D22 | 2026-09-19 | Wiring feedback is part of the product: `status()`, `verbose`, `selfTest()`, `doctor`, MCP `self_test` | Edward: "how does the user know when the framework has been properly equipped? that feedback is important." An idle wrapper is invisible below the threshold. | Documentation only |
